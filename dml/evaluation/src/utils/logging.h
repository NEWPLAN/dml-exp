 ...
 * \file logging.h
 * \brief defines logging macros of NEWPLAN
 *  allows use of GLOG, fall back to internal
 *  implementation when disabled
 */
#ifndef __NEWPLAN_LOGGING_H_
#define __NEWPLAN_LOGGING_H_
#include <cstdio>
#include <cstdlib>
#include <stdexcept>
#include <string>
 ...
 *  default logger if NEWPLAN_LOG_FATAL_THROW == 1
 */
...
#if NEWPLAN_USE_CXX11
#define NEWPLAN_THROW_EXCEPTION noexcept(false)
#else
#define NEWPLAN_THROW_EXCEPTION
#endif

//#define NEWPLAN_USE_GLOG 1
...
#define CHECK(x)                                                         \
  if (!(x))                                                              \
  NEWPLAN::LogMessageFatal(__FILE__, __LINE__).stream() << "Check "      \
                                                           "failed: " #x \
                                                        << ' '
...
#define CHECK_NOTNULL(x) \
  ((x) == NULL ? NEWPLAN::LogMessageFatal(__FILE__, __LINE__).stream() << "Check  notnull: " #x << ' ', (x) : (x)) // NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
#define DCHECK(x) \
  while (false)   \
...
#define LOG_INFO NEWPLAN::LogMessage(__FILE__, __LINE__)
#define LOG_ERROR LOG_INFO
#define LOG_WARNING LOG_INFO
#define LOG_FATAL NEWPLAN::LogMessageFatal(__FILE__, __LINE__)
...
#define LOG_IF(severity, condition) \
  !(condition) ? (void)0 : NEWPLAN::LogMessageVoidify() & LOG(severity)

#ifdef NDEBUG
#define LOG_DFATAL LOG_ERROR
#define DFATAL ERROR
#define DLOG(severity) true ? (void)0 : NEWPLAN::LogMessageVoidify() & LOG(severity)
#define DLOG_IF(severity, condition) \
  (true || !(condition)) ? (void)0 : NEWPLAN::LogMessageVoidify() & LOG(severity)
#else
...
#endif
  {
    log_stream_ << "[" << get_tid() << "] [" << pretty_date_.HumanDate() << "] " << file << ":" << line << ": ";
  }
...
inline std::string StackTrace()
{
  ...
  const int MAX_STACK_SIZE = NEWPLAN_LOG_STACK_TRACE_SIZE;
  ...
  if (msgs != nullptr)
  {
    ...
    {
      string msg = NEWPLAN::Demangle(msgs[frameno]);
...
#else // NEWPLAN_LOG_STACK_TRACE is off
...
inline std::string StackTrace()
{
  return std::string(
      ...
      "NEWPLAN_LOG_STACK_TRACE is disabled at compile time.");
}

#endif // NEWPLAN_LOG_STACK_TRACE

#if NEWPLAN_LOG_FATAL_THROW == 0
...
public:
  ...
  ~LogMessageFatal() NEWPLAN_THROW_EXCEPTION
  {
#if NEWPLAN_LOG_STACK_TRACE
    log_stream_ << "\n\n"
                << StackTrace() << "\n";
