...
void RDMABase::log_info(const char *format, ...)
{
    ...
    sprintf(now_time, "[%02d:%02d:%02d] ", tmnow->tm_hour,
...
struct RDMAContext *RDMABase::get_or_build_ctx(struct rdma_cm_id *id)
{
                             ...
                             struct ibv_qp_init_attr *qp_attr)
{
    ...
    qp_attr->cap.max_send_wr = MAX_DATA_IN_FLIGHT * 2 + 10 + 1;
    qp_attr->cap.max_recv_wr = MAX_DATA_IN_FLIGHT * 2 + 10 + 1;
...
void RDMABase::show_performance(int time_duration)
{
    ...
    printf("Total recv rate: %.2f Gbps\n",
