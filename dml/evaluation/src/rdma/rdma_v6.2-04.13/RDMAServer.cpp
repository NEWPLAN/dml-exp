...
void RDMAServer::on_pre_conn(struct rdma_cm_id *id)
{
    ...
    LOG_INFO("Server register buffer: \nblock size: %d, \nblock number: %d, \nbase address: %p\n",
             BUFFER_SIZE,
             MAX_DATA_IN_FLIGHT,
             ctx->buffer);
}
...
void RDMAServer::send_message(struct rdma_cm_id *id, uint32_t token_id)
{
    ...
    //wr.wr_id = (uintptr_t)id;
    wr.wr_id = token_id; //newplan debug used.
            ...
            uint32_t base_pos, int32_t num)
{
    ...
    memset(id, 0, 256 * sizeof(uint32_t));
    while (num > 0)
    {
        uint64_t code = 1;
        if (start < 64)
        ...
        {
            if (key[1] & code << (start - 64))
                *id = start;
        }
        else if (128 <= start && start < 192)
        {
            if (key[2] & code << (start - 128))
                *id = start;
        }
        else if (192 <= start && start < 256)
        {
            if (key[1] & code << (start - 192))
                *id = start;
        }
        else
        {
