...
void RDMAClient::on_pre_conn(struct rdma_cm_id *id)
{
    ...
    ret = posix_memalign((void **)&ctx->buffer, sysconf(_SC_PAGESIZE), BUFFER_SIZE);
    ...
    TEST_Z(ctx->buffer_mr = ibv_reg_mr(rc_get_pd(id), ctx->buffer, BUFFER_SIZE, 0));

    for (int msg_index = 0; msg_index < MAX_DATA_IN_FLIGHT; msg_index++)
    {
        ret = posix_memalign((void **)&ctx->msg[msg_index], sysconf(_SC_PAGESIZE), sizeof(struct message));
        TEST_Z(ctx->msg_mr[msg_index] = ibv_reg_mr(rc_get_pd(id), ctx->msg[msg_index], sizeof(struct message), IBV_ACCESS_LOCAL_WRITE));
        if (ret)
        {
            fprintf(stderr, "posix_memalign: %s\n",
                    strerror(ret));
    ...
    LOG_INFO("Client egister buffer: %p\n", ctx->buffer);
}
